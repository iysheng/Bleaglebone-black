...
{
    char *buf = "open";
	printk("bbb's led gpio60 will open.\n");
    file->private_data = led_devp;
	sprintf(led_devp->state,"%s","on");
	printk("opening led %s.\n",led_devp->state);
    return 0;
}
static ssize_t led_write (struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    int val;
	printk("write bbb's led.\n");
	struct led_dev *dev = file->private_data;
	copy_from_user(&val, buf, count);

	if (val == 1)
		{
		    //sprintf(dev->state,"%s","on");
			strcpy(dev->state,"on");
